...
## 6. 데이터 출처
- 예시 더미 노트는 제거됨. 실제 지식 베이스는 \`세계관/\`(로컬 전용, git 미포함)에서 \`scripts/import-worldbuilding.mjs\` 로 임포트해 \`src/seed-worldbuilding.local.js\` 로 생성, 첫 실행 시 시드됩니다.
- 단순 복사가 아니라 **하나의 무림 세계**로 재편됩니다: \`세계관/\`은 \`분류/시대\`(세계설정·세력·인물·무공·사건·지역·영물신물 × 과거·현재·양시대) 구조로 정리되고, \`00_정전\`의 정전(正典) 코덱스(세계 개요·무림 연표·힘의 체계·형이상학·세력 지도·무공 보감·계보·정합성 노트)가 두 시대를 잇습니다. 분류별 색인이 그래프를 끊김 없이 연결합니다. 성인 에셋·AI 모드·시스템 프롬은 제외됩니다.
- 구조 재편은 \`scripts/reorganize-world.mjs\`(1회), 이후 시드 생성은 \`scripts/import-worldbuilding.mjs\`. 세계관을 바꿨으면 import 재실행 → 재빌드.
